...
**Fase en curso:** Ninguna — **las 11 fases planificadas en `AGENTS.md` están cerradas**. El sitio está en producción real y completamente operativo en `https://economyandfaircompetition.com` (Vercel): CI/CD (`ci.yml`/`load-test.yml`), imágenes en Vercel Blob, correo real vía Resend, dirección/mapa reales en Contacto, y todos los textos institucionales (incluido el footer y las estadísticas del hero) editables desde el admin. Desarrollo local y producción usan bases de Mongo separadas (Mongo local en Docker para dev, Atlas exclusivo para producción) — ver `INFRA.md`.

**Reporte abierto, pendiente de diagnóstico con capturas en la próxima sesión:** el usuario reportó que "se perdió el estilo de los títulos grandes" en el sitio — la sesión anterior investigó un reporte similar (imágenes + títulos) y encontró/confirmó solo el problema real de imágenes (la URL de `home.hero.image` en Mongo había vuelto a una ruta local rota, ya corregida). El CSS de producción se verificó vía `curl` en ese momento y las clases de tipografía (`font-display`, `text-display-2xl`, etc.) sí estaban presentes y compiladas correctamente — el usuario confirmó entonces que tras recargar sin caché ambos problemas (imágenes y títulos) se veían bien. Si el reporte de títulos reaparece de forma persistente (no explicado por caché), **pedir capturas de pantalla antes de asumir la causa** — no se identificó ningún cambio de código en esa sesión que debiera afectar tipografía de títulos (los cambios fueron: footer convertido a Server Component, `EditableText` con soporte `dt`/`dd`, fix de la imagen del hero en Mongo). Revisar primero si el reporte es específico a alguna página/sección en particular, o general a todo el sitio.
...
**Incidente posterior — `home.hero.image` revertida a ruta local rota, mismo patrón de riesgo ya documentado antes**: el usuario reportó que las imágenes de Home volvieron a no cargar tras los deploys de esta entrada. Diagnóstico: `content_items.imageUrl` seguía correctamente en Blob (URLs `https://...blob.vercel-storage.com/...`), pero `site_texts["home.hero.image"]` había vuelto a `/uploads/home/home-hero-default.png` (ruta relativa, 404 real en Vercel). El registro en Mongo mostraba `updatedBy: "admin@economyandfaircompetition.com"` con timestamp dentro de la ventana en la que se corrió `seed-config.ts` y se verificó visualmente el fix del hero/footer en esta misma sesión — la vía más probable es la misma clase de riesgo ya registrada en una entrada anterior: navegar contra el servidor de desarrollo local con sesión de admin activa, mientras ese servidor apunta al mismo cluster Mongo Atlas de producción (no hay un Mongo separado por entorno), permitió que una interacción de verificación escribiera sobre el dato real sin intención. No se identificó con certeza el paso exacto (no quedó registrado qué comando lo causó), pero se descartó que fuera una edición manual real del usuario (confirmado explícitamente). Corregido re-apuntando `home.hero.image` a la URL de Blob ya confirmada como existente (`head` 200 antes de aplicar el cambio). **Lección reforzada para futuras sesiones**: evitar interactuar con `EditableText`/cualquier control de edición mientras se navega localmente en modo admin contra este mismo cluster — usar siempre un contexto de Playwright sin `storageState` (sin sesión) para verificaciones visuales de solo lectura, igual que la lección ya registrada tras el incidente de daño de datos documentado en la entrada de Fase 10.

## 2026-08-08 — Bases de datos separadas: dev ya no comparte cluster con producción

A raíz del incidente anterior (dev tocando accidentalmente un dato real de producción por compartir el mismo cluster Atlas), el usuario pidió separar los entornos: un Mongo local para desarrollo, dejando Atlas exclusivo para producción.

- **Mongo local**: se identificó un contenedor Docker ya existente en la máquina del usuario, `magic-link-mongo` (imagen `mongo:7`, mismo patrón que el Mailpit compartido del entorno), expuesto en el puerto `27019`→`27017`, autenticado con `admin`/`magiclink123`. No fue necesario crear infraestructura nueva, solo usar lo que ya corría.
- `.env` actualizado: `MONGODB_URI=mongodb://admin:magiclink123@localhost:27019/?authSource=admin`, `MONGODB_DB=economy-and-fair-competition-dev` (nueva base, separada de `economy-and-fair-competition-db` que sigue siendo la de Atlas/producción).
- **Snapshot real de Atlas → Mongo local**: `npm run seed:schema` creó las colecciones/validators en la base local nueva; un script puntual (con el driver de Mongo, solo lectura sobre Atlas — nunca escritura ahí) copió el contenido real de las 11 colecciones de Atlas a la base local: `content_items` (24), `site_texts` (180), `posts` (6), `contact_submissions` (14), `templates` (1), `circuit_breaker_state` (2), `uploaded_files` (1); `admin_users`/`ai_config`/`auth_codes`/`_connection_check` estaban vacías en Atlas y se omitieron. El script se descartó tras usarse (uso único).
- **`.env.prod` (nuevo archivo, gitignored)**: resguardo local de los valores reales configurados como Environment Variables en Vercel producción (Mongo Atlas, Resend, JWT, Anthropic, Blob) — para no depender de recordarlos si hace falta reconstruir algo. `vercel env pull` no trae los valores de las variables marcadas "Sensitive" (las devuelve vacías por diseño de seguridad de Vercel), así que se completó manualmente con los valores ya vistos/configurados en esta misma sesión. La fuente de verdad real sigue siendo el dashboard de Vercel.
- **Documentado**: `INFRA.md` (nueva sección "Bases de datos separadas dev/producción", explicando el porqué y el efecto de mitigación estructural que tiene sobre el tipo de incidente que lo motivó), `QUICK-START.md` (sección "Base de datos" actualizada con el comando para arrancar el contenedor si está detenido, y una nota explícita de que clonar datos reales de Atlas a local requiere confirmación porque toca el cluster de producción aunque sea solo lectura), `.env.example` (comentario explicando la opción recomendada de Mongo local vs. la alternativa de Atlas directo).
- **Validado**: dev server reiniciado contra la base local, confirmado sirviendo contenido real clonado (`curl` mostrando títulos reales de `content_items`). Vitest (35/35) y Playwright (16/16) en verde completos, incluyendo el flujo de login admin real (que depende de que la base local tenga las colecciones/índices correctos) — confirma que el aislamiento funciona de punta a punta sin romper nada del flujo de desarrollo existente.
